...
def get_kernel_knn(dataname, kernel_type, knn_nei_num, jug):
    ...
    if(os.path.exists(kernel_file)):
        ...
        dataset = fetch_dataset(dataname, verbose=False,data_home=f"D:/code-bucket/gnn/G2GNN-main/grak/",download_if_missing=True)
...
def get_class_num(imb_ratio, num_train, num_val):  # 你的长尾是自己取的。根据这个类别，这个train样本 数量，val样本数量进行选择。 返回一个【数量，单边-数量】的列表。
...
def construct_context_query(dataset):
    ...
    for i in range(len(classes)):
        ...
        clss_li.append(num_class_graph[i])

    chosen = torch.tensor(chosen)
    extend_data = dataset[chosen]

    data = list(dataset) + list(extend_data)  # 为了均衡化采样附在整个数据集后面的。如果传入下一阶段，将会无从再区分。

    return data, list(extend_data)
