import argparse


def parse_args():
    parser = argparse.ArgumentParser()

    # dataset
    parser.add_argument("--dataset", type=str, default="DD",
                        help="Choose a dataset:[MUTAG, PROTEINS, DHFR, DD, NCI1, PTC_MR, REDDIT-BINARY]")

    # model
    parser.add_argument('--n_hidden', type=int, default=128)
    parser.add_argument('--dropout', type=float, default=0.5)
    parser.add_argument('--prop_epochs', type=int, default=3)

    # experiments
    parser.add_argument('--runs', type=int, default=10)
    parser.add_argument('--epochs', type=int, default=200)
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--lr', type=float, default=0.01)
    parser.add_argument('--weight_decay', type=float, default=0.1)
    parser.add_argument('--method', type=str, default="ours")
    parser.add_argument('--batch_size', type=int, default=64)
    parser.add_argument('--imb_ratio', type=float, default=0.1)
    parser.add_argument('--num_train', type=int, default=300)
    parser.add_argument('--num_val', type=int, default=300)
    parser.add_argument('--early_stopping', type=int, default=100)

    # problem-specific
    parser.add_argument('--kernel_type', type=str, default='SP')
    parser.add_argument('--smote_k', type=int, default=10)
    parser.add_argument('--drop_edge_ratio', type=float, default=0.01)
    parser.add_argument('--mask_node_ratio', type=float, default=0.01)
    parser.add_argument('--aug_num', type=int, default=2)
    parser.add_argument('--temp', type=float, default=0.5)
    parser.add_argument('--knn_layer', type=int, default=2)
    parser.add_argument('--knn_nei_num', type=int, default=2)
    parser.add_argument('--gpu', type=int, default=0)
    parser.add_argument('--size_ratio', type=float, default=0.5)
    parser.add_argument('--hops', type=int, default=1)
    parser.add_argument('--deg_ratio', type=float, default=0.7)
    parser.add_argument('--aug_ratio', type=float, default=0.7)
    parser.add_argument('--use_drop', action='store_true')

    return parser.parse_args()
